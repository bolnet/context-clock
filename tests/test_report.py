...
def _rows():
    return [
        ...
        TurnRow(turn=3, context_tokens=406, cumulative_tokens=1677, recall=1.0,
                ...
                turn_cost=0.0006, cumulative_cost=0.0006),
        TurnRow(turn=8, context_tokens=976, cumulative_tokens=7500, recall=None,
                ...
                turn_cost=0.0011, cumulative_cost=0.0017),
    ]
...
class TestWriteCsv:
    ...
    def test_header_includes_cost_columns(self):
        # Real billed dollars must be in the CSV so the report's $ figures are
        # measured, not recomputed from list price.
        assert "turn_cost" in CSV_HEADER
        assert "cumulative_cost" in CSV_HEADER

    def test_rows_serialize_token_and_cost_columns(self, tmp_path):
        path = tmp_path / "out.csv"
        write_csv(_rows(), path)
        lines = path.read_text().strip().splitlines()
        # turn,context,cumulative,turn_tokens,prompt,completion,recall,compaction,turn_cost,cumulative_cost
        assert lines[1] == "1,164,167,167,132,35,,0,0.0,0.0"          # no cost → 0.0
        assert lines[2] == "3,406,1677,600,560,40,1.0,0,0.0006,0.0006"
        assert lines[3] == "8,976,7500,2200,900,20,,1,0.0011,0.0017"
